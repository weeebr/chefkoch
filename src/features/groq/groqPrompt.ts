...
export function buildOneShotJsonRecipeUserPrompt(
  ...
  const instruction = [
    ...
    `ingredientsOnHand, ingredientsShopping, spices, requiredBaseStaples,`,
    ...
    `- spices: Array gleicher Element-Typ wie ingredientsOnHand; jede im Gericht verwendete Gewürze-Zeile mit Menge; Salz/Pfeffer gehören zur impliziten Basis und nicht hierher. Nie in ingredientsOnHand/ingredientsShopping duplizieren.`,
    `- requiredBaseStaples: string[] mit kanonischen Namen nur aus: Salz, Pfeffer, Öl, Mehl, Zucker, Pasta, Reis, Butter, Milch — nur die die dieses Rezept wirklich braucht. Diese Namen nie in ingredientsOnHand oder ingredientsShopping wiederholen.`,
    `- Alle Zeitfelder sind ganze Zahlen >= 0.`,
    `- Zeitlogik strikt einhalten: minutes = prepMinutes + cookMinutes.`,
    `- Nutze den ZUTAT-Status für realistische Zeiten: "vorgekocht" reduziert prepMinutes deutlich; "roh / frisch" erhöht prepMinutes.`,
    `- Bei vielen "roh / frisch"-Zutaten darf prepMinutes nicht unrealistisch klein sein.`,
    `- Bei überwiegend "vorgekocht" darf prepMinutes kurz sein, aber cookMinutes bleibt für Garprozess realistisch.`,
